# UI Zone Next Developer Guide

Next.js product zone adapter.
...
- You need **zone composition**, **pluggable regions**, **next-generation UI assembly** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/ui-zone-next` instead of reaching into app-specific internals.
...
| `framework/libraries/ui-zone-next` | Nested publishable library package. |
| `framework/libraries/ui-zone-next/src` | Public runtime source and exported modules. |
| `framework/libraries/ui-zone-next/tests` | Unit and contract verification where present. |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `createNextZone` |
...
- Hosts should import from `@platform/ui-zone-next`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/ui-zone-next"]
...
```ts
import { packageId, packageDisplayName, createNextZone } from "@platform/ui-zone-next";

console.log(packageId, packageDisplayName);
console.log(typeof createNextZone);
```
...
- Publishes 0 public modules from `@platform/ui-zone-next`.
- Exports 4 named symbols through the public entrypoint, including `packageId`, `packageDisplayName`, `packageDescription`, `createNextZone`.
